...
//------------------------------------------------------------------------------
void VISLAMWorkerFunc( mvVISLAM * tracker , mvVISLAMPose &pose )
{
   while( !shutdownApp )
   {
      {
      ...
      if( gApplicationMode != CAPTURE_ONLY && vioFrameId >= gDelayVIOStart )
      {
         ...
         vi_pose = mvVISLAM_GetPose( tracker );
         char szResult[256];
   ...
   printf( "VISLAMWorkerFunc thread terminated and return vi estimate pose\n" );
...

{
   ...
   // Build a workerThread to run VISLAMWorkerFunc with gVISLAMTracker input, and return vi Pose
   std::thread workerThread( VISLAMWorkerFunc, gVISLAMTracker, std::ref(vi_pose) );
